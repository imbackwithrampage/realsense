...
{
    FrequencyDiagnostics(double expected_frequency, std::string name, std::string hardware_id) : expected_frequency_(expected_frequency),
  ...
  private:
    class CIMUHistory
    {
      public:
        ...
        {
          public:
            imuData(const imuData &other) : imuData(other.m_reading, other.m_time){};
            imuData(const float3 reading, double time) : m_reading(reading),
                                                         m_time(time){};
